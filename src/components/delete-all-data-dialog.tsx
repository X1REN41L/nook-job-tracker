...
export function DeleteAllDataDialog({ deleting, onCancel, onConfirm, returnFocusRef }: {
  ...
  return <div className="motion-dialog-backdrop fixed inset-0 z-[70] flex items-center justify-center bg-modal-backdrop/60 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget && !deleting) onCancel(); }}>
    <section ref={dialogRef} aria-describedby="delete-all-description" aria-labelledby="delete-all-title" aria-modal="true" className="motion-dialog-panel w-full max-w-md rounded-nook-lg border border-line bg-paper p-6 text-ink shadow-nook-lift outline-none" role="alertdialog" tabIndex={-1}>
